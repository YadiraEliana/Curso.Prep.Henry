...
function promedioResultadosTest(resultadosTest) {
  ...
  for(var i = 0; i < resultadosTest.length; i++) {
    plus = plus + resultadosTest[i]
  } 
  prom = plus / resultadosTest.length
  return prom
}
...
function mesesDelAño(array) {
  ...
  // Tu código:
  var arrayMesesEncontrados = [];
  for(var i = 0; i < array.length; i++){
    if(array[i] === "Enero" || array[i] === "Marzo" || array[i] === "Noviembre"){
      arrayMesesEncontrados.push(array[i]);
    }
  }
  if(arrayMesesEncontrados.length === 3){
    return arrayMesesEncontrados;
  }else{
